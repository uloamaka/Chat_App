...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  useEffect(() => {
    socket = io(ENDPOINT);
    socket.emit("setup", user.data.data.user);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      toast({
        title: "Error occured!",
        description: `Failed to load the  Results: ${error}`,
        status: "error",
        duration: 5000,
        isClosable: true,
        position: "bottom-left",
    ...
    <>
      {selectedChat ? (
        <>
          ...
          >
            ...
            {!selectedChat.isGroupChat ? (
              <>
                {getSender(user.data.data, selectedChat.participants)}
                <ChatProfileModal
                  user={getSenderFull(
                    user.data.data,
          ...
          >
            ...
            <FormControl mt={3}>
              <InputGroup>
                {isTyping &&  !isMessageSender(messages) ? (
                  <div>
                    <Lottie
        ...
        <Flex alignItems="center" justifyContent="center" height="100%">
          <Text fontSize="1xl" fontFamily="Work Sans" textAlign="center">
            <div>
              <strong>myChatApp</strong>
            </div>
            Send and receive messages super fast.
          </Text>
        </Flex>
      )}
    </>
